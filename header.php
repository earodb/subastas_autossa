        ...
        if($signo == "-"){
          ...
          if($comprador != null){//Si si tiene un ofertante se pasa a su cesta y cambia el estado de la subasta
              ...
              if($res_2 == false){
                ...
                $res2 = $bd->select("SELECT * from oferta where id_subasta=$id_subasta");
                if($res2->num_rows > 0){
                  while($row2 = $res2->fetch_assoc()){
                    $id_oferta = $row2["id_oferta"];
                    
                    $res_2_2 = $bd->query("UPDATE oferta set estado=1 where id_oferta=$id_oferta;");
                  }
                }
                if($res_2_1 == false){
                  echo "<script>alert('Estamos manejando errores');</script>";
              ...
              $res_3 = $bd->query("UPDATE subasta set estado=1, comprador=0 where id_subasta=$id_subasta;");
              if($res_3 == false){
                echo "<script>alert('Estamos manejando errores');</script>";
              }
          }
